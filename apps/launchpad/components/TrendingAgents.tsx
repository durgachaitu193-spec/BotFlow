      ...
      <div className="w-full overflow-hidden">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="min-w-[240px] h-[100px] rounded-xl border border-white/5 bg-bg-card animate-pulse"
            />
          ))}
        </div>
      </div>
          ...
          <Link href={`/agent/${agent.id}`} key={agent.id} className="block">
            ...
            >
              ...
              <div className="flex items-end justify-between">
                ...
                <div
                  className={`flex items-center text-xs font-medium ${agent.change.startsWith("+") ? "text-status-success" : "text-status-error"}`}
                >
