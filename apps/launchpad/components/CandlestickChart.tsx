...
const generateCandlestickData = (days: number) => {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    volumeData.push({
      time: time as Time,
      value: Number.parseFloat(volume.toFixed(0)),
...
export default function CandlestickChart({ id }: CandlestickChartProps) {
  ...
  useEffect(() => {
    ...
    if (id && chartData.length > 0) {
      ...
      // Use dummy data
      const timeframeDays = timeframes.find((tf) => tf.label === selectedTimeframe)?.days || 30
      const { candlestickData, volumeData } = generateCandlestickData(timeframeDays)
  ...
  return (
    <div className='flex h-full w-full flex-col'>
      ...
      <div ref={chartContainerRef} className='min-h-[300px] w-full flex-1 rounded-lg' />
    </div>
  )
}
