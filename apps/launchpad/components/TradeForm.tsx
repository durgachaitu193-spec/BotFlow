...
export default function TradeForm({ agent }: TradeFormProps) {
  ...
  const fetchBalance = async () => {
    ...
    try {
      ...
      if (mode === 'buy') {
        ...
        const contract = getDynamicBondingCurveTokenContract(agent.contractAddress, signer)
  ...
  useEffect(() => {
    const estimate = async () => {
      ...
      try {
        ...
        const contract = getDynamicBondingCurveTokenContract(agent.contractAddress, provider)
        ...
        if (mode === 'buy') {
          ...
          const receive = await contract.calculateTotalSellingCost(totalSupply, amountBN)
  ...
  const handleTrade = async () => {
    ...
    if (!amount || Number.isNaN(Number(amount)) || !agent.contractAddress) {
    ...
    try {
      ...
      toast.success(`Successfully ${mode === 'buy' ? 'bought' : 'sold'} ${amount} ${agent.ticker}`)
  ...
  return (
    ...
    >
      <div className='mb-6 flex rounded-lg bg-bg-surface p-1'>
        <button
          onClick={() => setMode('buy')}
          className={`flex-1 rounded-md py-2 font-bold text-sm transition-all ${
            mode === 'buy'
      ...
      <div className='space-y-4'>
        <div className='space-y-2'>
          ...
          <div className='relative'>
            <input
              ...
              placeholder='0'
              className='w-full rounded-lg border border-white/10 bg-bg-surface px-4 py-3 font-bold text-lg text-white placeholder:text-white/20 focus:border-accent-primary focus:outline-none'
            />
            <div className='-translate-y-1/2 absolute top-1/2 right-4 font-bold text-text-muted text-xs'>
              {agent.ticker}
            </div>
          </div>
          <div className='flex gap-2'>
            {['reset', '10', '100', '1000'].map((val) => (
              <button
                ...
                className='rounded bg-white/5 px-2 py-1 font-medium text-text-secondary text-xs hover:bg-white/10 hover:text-white'
              >
        ...
        <div className='space-y-2'>
          ...
          <div className='relative'>
            <input
              ...
              className='w-full rounded-lg border border-white/10 bg-bg-surface/50 px-4 py-3 font-bold text-lg text-text-muted focus:outline-none'
            />
            <div className='-translate-y-1/2 absolute top-1/2 right-4 font-bold text-text-muted text-xs'>
              BNB
            </div>
        ...
        <button
          ...
          className={`w-full rounded-xl py-3 font-bold text-sm text-white shadow-lg transition-all hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-50 ${
