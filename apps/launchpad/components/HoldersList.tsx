...
import { getHolders, type Holder } from '@/actions/getHolders'
  ...
  return (
    ...
    >
      <div className='mb-4 flex items-center justify-between'>
        ...
        <button className='text-accent-primary text-xs hover:text-accent-primary/80'>
          Generate bubble map
        </button>
      </div>
      <div className='space-y-1'>
        {isLoading && holders.length === 0 ? (
          <div className='py-4 text-center text-text-muted text-xs'>Loading holders...</div>
        ) : holders.length === 0 ? (
          <div className='py-4 text-center text-text-muted text-xs'>No holders found</div>
        ) : (
