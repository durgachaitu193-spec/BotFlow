...
export function formatBnBalance(value: BN | string, options: Options, network?: string): string {
  const { tokenDecimals } = networkConstants[network || CURRENT_NETWORK]
  ...
  const unit = withUnit ? ` ${networkConstants[network || CURRENT_NETWORK]?.tokenSymbol}` : ''
