...
import { IChartData, ITransaction } from "../types";
...
function aggregateByMinute(
  data: { createdAt: Date; currentPrice: BN }[],
): IChartData[] {
  ...
  return Object.entries(groupedByMinute)
    ...
    .map(([minuteKey, entries]) => {
      ...
      return {
        ...
        y: formatBnBalance(open, { numberAfterComma: 5 }, CURRENT_NETWORK)
          .split(",")
          .join(""),
      };
    });
