...
const styles = {
  ...
  headerStyle: 'w-full px-4 py-2 bg-black text-white sticky top-0 left-0 z-[100]',
...
function Layout({ children }: PropsWithChildren) {
  ...
  const formatDate = (date: string | { seconds: number } | { toDate: () => Date }): string => {
    if (!date) return ''
      ...
      if (response.ok) {
        ...
        setTxns(
          transactionsArray
            ...
            .sort((a: any, b: any) =>
              dayjs(a.createdAt, 'DD/MM/YYYY').isBefore(dayjs(b.createdAt, 'DD/MM/YYYY')) ? 1 : -1
            )
        )
      }
    } catch (error) {
          ...
          return {
            component: (
              <span
                ...
                } whitespace-nowrap pr-5 font-bold text-black`}
              >
                {item.type === 'created' ? (
                  <>
                    <Address address={item.to} startChars={4} endChars={4} /> created $
                    {item.tokenId} on {item.createdAt}
                  </>
                ) : (
                  <>
                    <Address address={item.to} startChars={4} endChars={4} />{' '}
                    {item.type === 'sell' ? 'sold' : 'bought'} $
                    {item.symbol ? item.symbol : item.tokenId} on {item.createdAt}
                  </>
                )}
              </span>
            ),
  ...
  return (
    <AntLayout className={layoutStyle}>
      ...
      <Content style={{ minHeight: 'calc(100vh - 50px - 64px)' }} className={contentStyle}>
