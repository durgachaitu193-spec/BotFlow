...
function AddressDropdown({ disabled, label, wallet, onAddressSelect }: Props) {
  ...
  const [selectedAddressName, setSelectedAddressName] = useState<string | null>(
    null,
  );
  ...
  const handleSetSigner = async (apiPromise: ApiPromise) => {
    ...
    const injected =
      injectedWallet &&
      injectedWallet.enable &&
      (await injectedWallet.enable(APP_NAME));
    if (!injected) {
  ...
  return (
    <div className="flex w-full flex-col gap-3">
      ...
      <div className="flex w-full flex-col gap-1">
        ...
        >
          <div className="flex items-center gap-3 w-full border border-gray-700 p-2 rounded-lg">
