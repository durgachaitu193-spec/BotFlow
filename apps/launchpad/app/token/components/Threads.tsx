...
const Threads = ({ id, ticker }: { id: string; ticker: string }) => {
  ...
  return (
    <div ref={ref}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-x-3">
          <Button
            ...
            className={`${tab === ETab.THREADS ? "bg-primary text-black" : "bg-transparent text-white"} p-2 text-[16px] font-bold border-none`}
          >
            {ETab.THREADS}
          </Button>
          <Button
            onClick={() => setTab(ETab.TRADES)}
            className={`${tab === ETab.TRADES ? "bg-primary text-black" : "bg-transparent text-white"} p-2 text-[16px] font-bold border-none`}
          >
            {ETab.TRADES}
          </Button>
        </div>
      ...
      {tab === ETab.TRADES ? (
        <div className="flex flex-col gap-y-3 w-[full] overflow-x-auto">
          <div className="rounded-[12px] p-3 bg-bg_primary grid grid-cols-6 min-w-[650px]">
          ...
          {txns &&
            ...
            txns.map((item) => (
              <div
                key={item.txHash}
                className="rounded-[12px] p-3 bg-bg_primary grid grid-cols-6 min-w-[650px]"
              >
                ...
                <p className="col-span-1">
                  {(item.value / 10 ** 11).toFixed(4)}
                </p>{" "}
                <p className="col-span-1">{item.amount}</p>
                <p className="col-span-1">
                  {dayjs(item.createdAt).format("DD MMM, LT")}
                </p>
