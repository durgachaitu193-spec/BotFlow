...
import { CLIENT_DB_COLLECTIONS, Network } from "@/global/types";
...
const HolderDistribution = ({
  ...
  currentSupply,
  id,
}: IHolderDistributionProps) => {
  const [holderDistribution, setHolderDistribution] = useState<{
    [index: string]: number;
  }>({});

  const [sortedHolders, setSortedHolders] = useState<
  ...
  return (
    <div className="flex flex-col gap-y-3">
      <p className="font-bold text-[20px] text-white mb-1 text-heading flex items-center justify-between">
        Holder Distribution
        <span className="text-sm text-white">
          ({sortedHolders.length} wallets)
        </span>
      </p>
      ...
      {sortedHolders.length > 5 && (
        <p
          ...
          className="text-primary text-sm font-bold cursor-pointer flex items-center gap-x-1"
        >
