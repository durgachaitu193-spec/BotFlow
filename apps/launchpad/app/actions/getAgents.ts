...
export async function getAgents(): Promise<Agent[]> {
  try {
    ...
    return agents.map((a) => {
      ...
      const name = String(
        metadata?.chatTitle || metadata?.tokenName || metadata?.name || 'Unknown Agent'
      ).toUpperCase()

      // Filter transactions specific to this agent (tokenId matches agent.id)
      const agentTransactions = allTransactions.filter((t) => t.tokenId === a.id)
      ...
      const recentActivity = agentTransactions.slice(0, 5).map((tx) => {
        ...
        return {
          type: tx.type,
          time: timeStr,
          details: `${actionType} ${Number.parseFloat(tx.amount).toFixed(2)} ${tx.symbol}`,
        }
      })
      ...
      return {
        ...
        image: metadata?.tokenIpfsHash
          ...
          : String(metadata?.image || `https://api.dicebear.com/7.x/bottts/svg?seed=${a.id}`),
        isPositive: true,
        status: a.isActive ? 'active' : 'inactive',
        description: String(metadata?.chatDescription || metadata?.description || ''),
...
export async function getAgentById(id: string): Promise<Agent | undefined> {
  try {
    ...
    const tokenTicker = String(metadata?.tokenSymbol || metadata?.ticker || 'UNKNOWN')
    const tokenAddress = String(metadata?.tokenAddress || result.agentWallet)
    const tokenImage = metadata?.tokenIpfsHash
      ? `https://gateway.pinata.cloud/ipfs/${String(metadata.tokenIpfsHash)}`
      : String(metadata?.image || `https://api.dicebear.com/7.x/bottts/svg?seed=${result.id}`)
