...
export const createPoolAndRemoveLiquidity = ({
  ...
  const createPoolTx = api.tx.xyk.createPool(
    asset1Id,
    asset1Amount,
    asset2Id,
    asset2Amount,
  ...
  const removeValue = new BN(asset1Amount)
    .sub(new BN(asset1Amount).div(new BN(1000)))
    .toString();
  const removeLiquidityTx = api.tx.xyk.removeLiquidity(
    asset1Id,
    asset2Id,
    removeValue,
  );
