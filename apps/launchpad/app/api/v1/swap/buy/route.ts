...
import { launchpadHoldings, launchpadTokens, launchpadTransactions } from '@botflow/db/schema'
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      const onFailed = async () => {
        const keyring = await getKeyringPairFromMnemonic(mnemonic)
        const tx = api.tx.balances.transferKeepAlive(address, transferMoney.toString())
      ...
      if (new BN(newReserveBalance).gte(DX_LIST_AMOUNT)) {
        tradeDisabled = true
        try {
          const updatedSupply = buyRequestedAmount.add(new BN(buyRequestedAmount).div(new BN(100)))
          migrateToHydraDX({
            sender: keyRing,
            assetId: id,
