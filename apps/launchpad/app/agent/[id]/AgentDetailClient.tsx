      ...
      <div className='space-y-6'>
        ...
        <div className='flex flex-wrap items-center justify-between gap-4 rounded-xl border border-white/5 bg-bg-card p-4'>
          <div className='flex items-center gap-4'>
            <div className='h-14 w-14 overflow-hidden rounded-lg border border-white/10 bg-bg-surface'>
              <img src={agent.image} alt='Agent Avatar' className='h-full w-full object-cover' />
            </div>
            <div>
              <div className='flex items-center gap-2'>
                <h1 className='font-bold text-white text-xl'>{agent.name}</h1>
              </div>
              <div className='flex items-center gap-2 text-sm'>
                <span className='font-bold text-white'>{agent.ticker}</span>
                <span className='rounded bg-white/5 px-1.5 py-0.5 text-text-secondary text-xs'>
              ...
              <div className='mt-1 flex flex-col gap-1 text-text-muted text-xs'>
                <div className='flex items-center gap-2'>
                  <img
          ...
          <div className='flex gap-2'>
            <a
              ...
              className='flex items-center gap-2 rounded-lg bg-accent-primary px-4 py-2 font-bold text-bg-deep text-sm hover:bg-accent-primary/90'
            >
        ...
        <div className='grid grid-cols-1 gap-6 lg:grid-cols-3'>
          ...
          <div className='space-y-6 lg:col-span-2'>
            ...
            <div className='flex flex-wrap items-end gap-8'>
              <div>
                <div className='mb-1 text-text-muted text-xs'>Market Cap</div>
                <div className='font-bold text-2xl text-white'>{agent.marketCap}</div>
                <div
              ...
              <div className='h-2 max-w-xs flex-1 self-center overflow-hidden rounded-full bg-bg-surface'>
                <div className='h-full w-[40%] bg-gradient-to-r from-status-success to-accent-primary' />
              </div>
              <div className='self-center font-bold text-sm text-white'>ATH {agent.fdv}</div>
            </div>

            {/* Chart */}
            <div
              className='h-[500px] overflow-hidden rounded-xl bg-bg-card p-1'
              style={{
                background:
          ...
          <div className='space-y-6'>
            ...
            <div className='rounded-xl border border-white/5 bg-bg-card p-6'>
              <div className='mb-2 flex items-center justify-between'>
                <h3 className='font-semibold text-sm text-white'>Bonding Curve Progress</h3>
                <span className='font-bold text-accent-primary text-sm'>100.0%</span>
              </div>
              ...
              <p className='text-text-muted text-xs'>Coin has graduated!</p>
            </div>

            {/* Chat Section - Only for chat agents */}
            {agent.deploymentType === 'chat' && agent.chatUrl && (
              <div className='mb-6 rounded-xl border border-[#30363D] bg-[#0D1117] p-6'>
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-4'>
                    <div className='h-12 w-12 overflow-hidden rounded-full bg-[#161B22]'>
                      <img
                        src={agent.image}
                        alt={agent.name}
                  ...
                  <a
                    ...
                    target='_blank'
                    rel='noopener noreferrer'
                    className='rounded-lg bg-[#1F242C] px-6 py-2 font-medium text-white transition-colors hover:bg-[#2D333B]'
                  >
                    Join chat
                  </a>
                </div>
