...
export const createToken = async ({
  ...
  try {
    ...
    console.log(
      "Step 2: Creating token on blockchain using tokenFactory helper...",
    );
...
export const calculateTotalCost = async ({
  ...
  const cost = await contract.calculateTotalCost(
    tokenName,
    tokenSymbol,
    amount,
  );
...
export const getAllTokenInformation = async () => {
  ...
  return tokenInfo.map(
    (info: { name: string; symbol: string; ipfsHash: string }) => ({
      name: info.name,
      symbol: info.symbol,
