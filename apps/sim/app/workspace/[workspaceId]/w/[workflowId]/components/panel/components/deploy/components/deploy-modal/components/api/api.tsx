...
export function ApiDeploy({
  ...
  const getBaseEndpoint = () => {
    if (!info) return ''
    return info.endpoint.replace(info.apiKey, '$MEGALITH_API_KEY')
  }
...
console.log(job); // Contains job_id for status checking`
      ...
      case 'status':
        switch (language) {
          case 'curl':
            return `curl -H "X-API-Key: $MEGALITH_API_KEY" \\
...
response = requests.get(
    "${baseUrl}/api/jobs/JOB_ID_FROM_EXECUTION",
    headers={"X-API-Key": MEGALITH_API_KEY}
)
