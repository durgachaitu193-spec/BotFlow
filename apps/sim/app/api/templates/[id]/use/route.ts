...
import { templates, workflow, workflowDeploymentVersion } from '@botflow/db/schema'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const accessCheckResponse = await fetch(`${getBaseUrl()}/api/templates/${id}/check-access`, {
      headers: {
    ...
    if (accessCheckResponse.ok) {
      ...
      if (!accessData.hasAccess) {
        logger.warn(`[${requestId}] User ${session.user.id} does not have access to template ${id}`)
      ...
      logger.warn(`[${requestId}] Access check failed for template ${id}, allowing use`)
    }
