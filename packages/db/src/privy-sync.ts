...
export async function syncPrivyUser(
  ...
  try {
    ...
    // 2. Extract Data
    const emailAccount = privyUserData.linkedAccounts?.find((account) => account.type === 'email')
    const userEmail = emailAccount?.address
    const userName = (privyUserData.metadata?.name as string) || userEmail?.split('@')[0] || 'User'
    ...
    try {
      if (userEmail) {
        const foundUsers = await db.select().from(user).where(eq(user.email, userEmail)).limit(1)
        existingUser = foundUsers[0] || null
      } else {
        // If no email, check if a user with the generated email already exists
        const sanitizedPrivyId = privyUserData.privyId.replace(/[^a-zA-Z0-9]/g, '').substring(0, 20)
    ...
    if (existingUser) {
      const isEmailVerified =
        emailAccount?.verifiedAt !== undefined && emailAccount?.verifiedAt !== null
          ? true
    ...
    try {
      const foundUsers = await db.select().from(user).where(eq(user.email, finalEmail)).limit(1)
